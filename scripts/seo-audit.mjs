// Crawls dist/ (must be built first) and checks every generated HTML page for:
// - missing/duplicate <title>
// - missing/duplicate meta description
// - missing or duplicate H1
// - missing canonical link
// - canonical mismatched with the page's own URL (unless intentional, e.g. redirect pages)
// - images missing alt attributes (excluding decorative ones already marked alt="")
// - JSON-LD blocks that fail to parse as JSON
// - noindex on pages that should be indexable (redirect pages are expected to be noindex)
//
...
for (const file of htmlFiles) {
  ...
  if (isRedirectPage) continue; // redirect stub pages are exempt from content SEO checks
  ...
  // --- noindex sanity (indexable pages shouldn't be noindexed by accident) ---
...
// --- Duplicate titles/descriptions across distinct URLs ---
...
console.log(`\nSEO audit: scanned ${htmlFiles.length} HTML files in dist/\n`);

...
// - every redirect target resolves to a real page in dist/
//
// Run after `astro build` (dist/ must exist). Exits non-zero on failure so
// it can gate CI/local QA.
...
if (!existsSync(distDir)) {
  console.error('dist/ not found — run `npm run build` before validating redirects.');
  process.exit(1);
}
...
for (const target of uniqueTargets) {
  if (!pageExists(target)) {
    errors.push(`Dead redirect target: "${target}" does not resolve to a page in dist/`);
  }
}
